...
if (!isset($_POST['user1age']) || empty($_POST['user1age'])) {
	$_POST['user1age'] = 0;
} elseif (($_POST['user1age'] < 0) || ($_POST['user1age'] > 100)) {
	$checkError++;
...
if (!isset($_POST['user2age']) || empty($_POST['user2age'])) {
	$_POST['user2age'] = 0;
} elseif (($_POST['user2age'] < 0) || ($_POST['user2age'] > 100)) {
	$checkError++;
...
if ($checkError === 0) {
	...
	$userWorker1->name = $_POST['user1name'];
	$userWorker1->age = $_POST['user1age'];
	$userWorker1->salary = $_POST['user1salary'];

	$userWorker2 = new UserWorker();
	$userWorker2->name = $_POST['user2name'];
	$userWorker2->age = $_POST['user2age'];
	$userWorker2->salary = $_POST['user2salary'];

	echo 'Сумма зарплат   ' . $userWorker1->name . ' и ' . $userWorker2->name . ': ' . ($userWorker1->salary + $userWorker2->salary) . "</br>";
	echo 'Сумма возрастов ' . $userWorker1->name . ' и ' . $userWorker2->name . ': ' . ($userWorker1->age + $userWorker2->age) . "</br>";
}
